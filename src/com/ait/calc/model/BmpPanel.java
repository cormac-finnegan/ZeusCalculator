...
public class BmpPanel {
	...
	public static int[][] returnOriginArray(int row, int column, String color){
	...
	public static int[][] returnDecompressedArray(int row, int column, String color){
		if(color.equalsIgnoreCase("Red")){
			return redD[row][column];
		}else if(color.equalsIgnoreCase("Green")){
			return greenD[row][column];
		}else{
			return blueD[row][column];
		}
	}
	...
	public static void convertToCompressedAndDecompressedArray(){
		...
		int qLevel = MatrixPanel.qSlider.getValue();
		/*		do {
	        while (!in.hasNextInt()){
	        	in.next();
	        } 
