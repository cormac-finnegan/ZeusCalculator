...
public class FourierFunctionChart {	
	...
	public FourierFunctionChart() {
		super();
	}

	public FourierFunctionChart(String minX, String maxX, String function){
		super();		
		try{minX=minX.replace("p", "3.14159265");
			maxX=maxX.replace("p", "3.14159265");
	...
	public void calculatePoint(){
		...
		for(int i=0; i< (X_INTERVAL+1) ; i++){
			double xValue = minX + (maxX-minX)* ((double)i) / ((double)X_INTERVAL);
			
			String inputToCalculator = function.replace("x", "(" + Double.toString(xValue) + ")");	
			inputToCalculator = inputToCalculator.replace("p",  "3.14159265" );
			ScientificCalculatorJavaForFourierLine calculator = new ScientificCalculatorJavaForFourierLine();
			calculator.setInput(inputToCalculator);
			double result = calculator.getResult();	
			PlotPoint plotPoint = new PlotPoint(xValue, result);
