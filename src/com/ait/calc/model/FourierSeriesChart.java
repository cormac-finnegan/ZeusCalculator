/*
 * comments out main()
 * it takes 7 paramenters in constructor()
 * return plotpoints arrayList   : public ArrayList<PlotPoint> getPlotPoints()
 * 
 */
...
//need PlotPoint.java, and ScientificCalculatorJava.java

public class FourierSeriesChart {
	
	static Scanner sc = new Scanner(System.in);
	private final int X_INTERVAL = 100;
	private String periodL;
	private double drawMinX;
	private double drawMaxX;
	private double a0;
	private int termK;
	private String functionAk;
	private String functionBk;
	private String functionSum;
	private ArrayList<PlotPoint> plotPoints = new ArrayList<PlotPoint>(); 
	// periodL, a0, functionAk, funcationBk, termK = L, A0, Ak, Bk, K in Fourier Series  
	
	
	
	public static void main(String[] args) {
		FourierSeriesChart fourier = new FourierSeriesChart();
		fourier.inputAllValue();
		fourier.getSumFunction();
		fourier.calculatePoint();		

	}//end main()
	
	public FourierSeriesChart() {
		super();
	}

	public FourierSeriesChart(String periodL, double drawMinX, double drawMaxX, double a0, int termK, String functionAk,
			String functionBk) {
		super();
		...
		this.a0 = a0;
		this.termK = termK;
		this.functionAk = functionAk;
		this.functionBk = functionBk;
	}//end constructor

	public FourierSeriesChart (String periodL, String drawMinX, String drawMaxX, String a0, String termK, String functionAk,
			String functionBk) {
		super();
		try{
			this.periodL = periodL;
			drawMinX=drawMinX.replace("p", "3.14159265");
			drawMaxX=drawMaxX.replace("p", "3.14159265");
			a0=a0.replace("p", "3.14159265");			
			this.drawMinX = Double.parseDouble(drawMinX);
			this.drawMaxX = Double.parseDouble(drawMaxX);
			this.a0 = Double.parseDouble(a0);;
			this.termK = Integer.parseInt(termK);;
			this.functionAk = functionAk;
			this.functionBk = functionBk;
		}catch(Exception e) {
			e.printStackTrace();
	...
	public void inputAllValue() {
		try {
			//value for create function
			System.out.println("Enter 1/2 period L");
			periodL = sc.next();		
			System.out.println("Enter a0");
			a0 = sc.nextDouble();				
			System.out.println("Enter function/value of Ak (x)");
			functionAk = "(" + sc.next() + ")";
			System.out.println("Enter function/value of Bk (x)");		
			functionBk = "(" + sc.next() + ")";
			
			//value for drawing fourier series
			System.out.println("Enter drawMinX");
			drawMinX = sc.nextDouble();		
			System.out.println("Enter drawMaxX");
			drawMaxX = sc.nextDouble();			
			System.out.println("Enter termK");
			termK = sc.nextInt();
		} catch (Exception e) {			
			e.printStackTrace();
		}
	...
	public void getSumFunction(){
		//f(t) = a0/2+sum( ak * cos( k*pi*t/L ) + bk * sin( k*pi*t/L )  )
		//this only get the sum part, without a0/2
		functionSum = functionAk + "*cos(k*3.14159265*x/" + periodL + ")+" + functionBk + "*sin(k*3.14159265*x/" +periodL + ")";
		System.out.println(functionSum);	
		
		
	}//end getFinalFunction()
	
	public void calculatePoint(){
		//calculate each (x,y), use 100 points
		for(int i=0; i< (X_INTERVAL+1) ; i++){
			double xValue = drawMinX + (drawMaxX-drawMinX)* ((double)i) / ((double)X_INTERVAL);
			
			//calcualte sum() value
			double sumTerms = 0;
			for(int j=0; j<termK; j++){
				String inputToCalculator = functionSum.replace("x", "(" + Double.toString(xValue) + ")" );
				inputToCalculator = inputToCalculator.replace("k",  "(" + Integer.toString(j+1) + ")" );
				inputToCalculator = inputToCalculator.replace("p",  "3.14159265" );
				//System.out.println(inputToCalculator);
				
				ScientificCalculatorJavaForFourier calculator = new ScientificCalculatorJavaForFourier();
				calculator.setInput(inputToCalculator);
				double result = calculator.getResult();	
				sumTerms += result;				
			}
			
			double yValue = a0/2 + sumTerms;
			System.out.println(" x = " + xValue + " y = " + yValue);
			PlotPoint plotPoint = new PlotPoint(xValue, yValue);
